-- ========================================
-- PARTNER COORDINATION SYSTEM - COMPLETE DEPLOYMENT SCRIPT
-- ========================================
-- 
-- This script deploys the complete partner coordination system.
-- It combines schema creation, RLS policies, and validation in the correct order.
-- 
-- Run this entire script in your Supabase SQL Editor to deploy the system.
--
-- Project: GymBuddy Partner Coordination
-- Database: https://cikoqlryskuczwtfkprq.supabase.co
-- 
...
-- Step 2: Create all new tables for partner coordination
...
CREATE INDEX IF NOT EXISTS idx_partner_requests_requested_user ON public.partner_requests(requested_user_id);  
CREATE INDEX IF NOT EXISTS idx_partner_requests_status ON public.partner_requests(status);

-- Session proposals indexes
CREATE INDEX IF NOT EXISTS idx_session_proposals_proposer ON public.session_proposals(proposer_id);
CREATE INDEX IF NOT EXISTS idx_session_proposals_partner ON public.session_proposals(partner_id);
CREATE INDEX IF NOT EXISTS idx_session_proposals_status ON public.session_proposals(status);
CREATE INDEX IF NOT EXISTS idx_session_proposals_date ON public.session_proposals(proposed_date);
...
CREATE INDEX IF NOT EXISTS idx_coordination_states_partners ON public.coordination_states(partner_1_id, partner_2_id);
CREATE INDEX IF NOT EXISTS idx_coordination_states_state ON public.coordination_states(state);

-- ========================================
-- CREATE HELPER FUNCTIONS
-- ========================================

-- Update coordination_states.updated_at on changes (use existing function)
DROP TRIGGER IF EXISTS update_coordination_states_updated_at ON public.coordination_states;
...
DROP TRIGGER IF EXISTS update_coordination_proposals_count_trigger ON public.session_proposals;
...
DROP POLICY IF EXISTS "service_role_full_access_partner_requests" ON public.partner_requests;
CREATE POLICY "service_role_full_access_partner_requests" ON public.partner_requests
    FOR ALL 
    USING (auth.role() = 'service_role');

-- Users can view requests they sent or received
DROP POLICY IF EXISTS "users_can_view_own_partner_requests" ON public.partner_requests;
...
DROP POLICY IF EXISTS "users_can_create_partner_requests" ON public.partner_requests;
...
DROP POLICY IF EXISTS "users_can_update_own_partner_requests" ON public.partner_requests;
...
-- Service role full access
DROP POLICY IF EXISTS "service_role_full_access_session_proposals" ON public.session_proposals;
...
DROP POLICY IF EXISTS "partners_can_view_session_proposals" ON public.session_proposals;
...
DROP POLICY IF EXISTS "partners_can_create_session_proposals" ON public.session_proposals;
...
DROP POLICY IF EXISTS "partners_can_update_session_proposals" ON public.session_proposals;
...
DROP POLICY IF EXISTS "service_role_full_access_notifications" ON public.notifications;
...
-- Service role full access
DROP POLICY IF EXISTS "service_role_full_access_coordination_states" ON public.coordination_states;
...
DROP POLICY IF EXISTS "partners_can_view_coordination_states" ON public.coordination_states;
...
DROP POLICY IF EXISTS "system_can_create_coordination_states" ON public.coordination_states;
CREATE POLICY "system_can_create_coordination_states" ON public.coordination_states
    FOR INSERT 
    WITH CHECK (auth.role() = 'service_role');

-- System can update coordination states
DROP POLICY IF EXISTS "system_can_update_coordination_states" ON public.coordination_states;
...
DROP POLICY IF EXISTS "users_can_view_coordination_partner_profiles" ON public.users;
...
DROP POLICY IF EXISTS "users_can_view_proposal_based_sessions" ON public.sessions;
...
SELECT 'Partner Coordination System deployed successfully!' as deployment_status;